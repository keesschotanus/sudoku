 ...
 * 
 * Currently this class only supports: n choose k.
 * Example, list all combinations of size 3, from the set {a,b,c,d,e}:
 * ```
 * const combination = new Combination<string>(['a', 'b', 'c', 'd', 'e'], 3);
 * while (combination.hasNext()) {
 *   console.log(combination.next());
 * }
 * ```
 * @typeParam T The type of combination.
 *  You can use numbers or strings and even objects.
 */
export default class Combination<T> {
  ...
  /**
   * For performance and memory consumption reasons we reuse the same array for all combinations.
   */
  private combination = new Array<T>();
